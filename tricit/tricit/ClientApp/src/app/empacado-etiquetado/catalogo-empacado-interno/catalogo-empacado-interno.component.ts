...
import { DialogoEliminarEmpacadoInternoComponent } from './dialogo-eliminar-empacado-interno/dialogo-eliminar-empacado-interno.component';
...
@Component({
  selector: 'app-catalogo-empacado-interno',
  templateUrl: './catalogo-empacado-interno.component.html',
  styleUrls: ['./catalogo-empacado-interno.component.less']
})
export class CatalogoEmpacadoInternoComponent implements OnInit {
  selectedValue: any;
    searchTxt: any;
  ...
  dialogoEliminarEmpacado() {
    ...
    let action: any = {
      mensaje: validacion == 1 ? "Esta seguro de eliminar el empacado interno seleccionado" : "Solo se puede eliminar empacados internos con estatus Inactivo", //Si no, solo mostrar el mensaje de error,
      tipo: validacion == 1 ? 0 : 1 //alguna validación para incitar en eliminar
    }

    let _dialogRef = this._dialog.open(DialogoEliminarEmpacadoInternoComponent, {
      panelClass: "dialog-aprod",
  ...
  */
  ...
  obtenerDatos(action = 0) {
    ...
    this._dataService.postData<any>("InternalPacked/SearchInternalPacked", sessionStorage.getItem("token"), data).subscribe(
      data => {
  ...
  seleccion = () => {
    this.selection.selected.map(item => { this.packId = item.packedId, this.status = item.status, this.empacadoName = item.packedName, this.empacadorActual = item });
    this.selection.selected.map(item => { this.direccionId = item.direccionId });
  }

  catalogoOperador() {

    this._router.navigateByUrl(`EmpacadoEtiquetado/EmpacadoInterno/Operadores/${1}/${this.packId}/${this.empacadoName}`, { queryParams: { proviene: 1, id: this.packId }, state: { proviene: 1, packedId: this.packId } });
  }

  catalogoLOperaciones() {
    this._router.navigateByUrl(`EmpacadoEtiquetado/EmpacadoInterno/LineasOperacion/${1}/${this.packId}/${this.empacadoName}`, { queryParams: { proviene: 1, id: this.packId }, state: { proviene: 1, packedId: this.packId } });
  }

  gestionCajas() {
    this._router.navigateByUrl(`EmpacadoEtiquetado/EmpacadoInterno/GestionCajas/${1}/${this.packId}/${this.empacadoName}`, { queryParams: { proviene: 1, id: this.packId }, state: { proviene: 1, packedId: this.packId } });
  }

  reporteArmados() {
    this._router.navigateByUrl(`EmpacadoEtiquetado/EmpacadoInterno/ReporteArmados/${1}/${this.packId}/${this.empacadoName}`, { queryParams: { proviene: 1, id: this.packId }, state: { proviene: 1, packedId: this.packId } });
  }

  dialogoAsociarProductos() {
    let _dialogRef = this._dialog.open(DialogoAsociarProductosComponent, {
      panelClass: "dialog-aprod",
      data: { titulo: "Agregar", type: 2, id: this.packId }
    });

    _dialogRef.afterClosed().subscribe(ref => {
      if (ref == true) {
        if (this.empacadoI != "" || this.razonS != "") {
          this.obtenerDatos(1);
        } else {
          this.obtenerDatos();
        }
  ...
  accionEnabled = () => {
    ...
    if (this.selection.selected.length == 0) {
      ...
      //el usuario logueado es tipo empacador interno?
      if (parseInt(sessionStorage.getItem("isType"), 10) == 2) {

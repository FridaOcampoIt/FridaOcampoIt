...
import { DialogoAgregarConfiguracionComponent } from './dialogo-agregar/dialogo-agregar.component';
import { DialogoEliminarConfiguracionComponent } from './dialogo-eliminar/dialogo-eliminar.component';
...
@Component({
  selector: 'app-configuracion-embalaje',
  templateUrl: './configuracion-embalaje.component.html',
  styleUrls: ['./configuracion-embalaje.component.less']
})
export class ConfiguracionEmbalajeComponent implements OnInit {

  familiaId: number = 0;
  embalajeId: number = 0;
  companyId:  number = 0;

  displayedColumns: string[] = ['select', 'packagingType', 'readingType', 'unitsPerBox', 'copiesPerBox', 'boxesPerPallet', 'linesPerBox', 'copiesPerPallet'];
  ...
  constructor(
    ...
    this.companyId = parseInt(this._route.snapshot.paramMap.get('companyId'), 10);
  }
  ...
  dialogoAgregar(action = 0) {
    ...
    const _dialogRef = this._dialog.open(DialogoAgregarConfiguracionComponent, {
      panelClass: 'dialog-aprod',
      disableClose: true,
      data: { familyId: this.familiaId, embalajeId: this.embalajeId, action: action, companyId: parseInt(this._route.snapshot.paramMap.get('companyId'), 10) }
    })
  ...
  dialogoEliminar() {
    const _dialogRef = this._dialog.open(DialogoEliminarConfiguracionComponent, {
      panelClass: 'dialog-aprod',
      disableClose: false,
  ...
  obtenerDatos(action = 0) {
    ...
    this._dataService.postData<any>("Families/getPackaging", sessionStorage.getItem("token"), data).subscribe(
      data => {
        setTimeout(() => {
        ...
        console.warn('data configuracion embalaje', this.dataSource.data);
      },
      error => {
